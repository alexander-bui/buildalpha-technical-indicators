...
def GetCustomSignal():
        ...
        ##
        df1['Cum_Vol'] = df1.iloc[:, 5].cumsum()
        df1['Cum_Vol_Price'] = (df1.iloc[:, 5] * (df1.iloc[:, 2] + df1.iloc[:, 3] + df1.iloc[:, 4] ) /3).cumsum()
        df1['VWAP'] = df1['Cum_Vol_Price'] / df1['Cum_Vol']
        
        Signal = [1 if c < v else 0 for c,v in zip(df1.iloc[:, 4], df1.VWAP)]
