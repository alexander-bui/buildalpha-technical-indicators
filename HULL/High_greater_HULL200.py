...
##
def calcHULL(close, length):
        data = close

        df1['WMA'] = 2* talib.WMA(data.values,(length/2))
        df1['WMA2'] = talib.WMA(data.values,length)  
        df1['Diff'] = df1['WMA'] - df1['WMA2']

        df1['HULL'] = talib.WMA(df1['Diff'].values,round(math.sqrt(length)))
...
def GetCustomSignal():
        ...
        ##
        calcHULL(df1.iloc[:, 4], 200)
        
        Signal = [1 if hi > h  else 0 for hi,h in zip(df1.iloc[:, 2], df1.HULL)]
