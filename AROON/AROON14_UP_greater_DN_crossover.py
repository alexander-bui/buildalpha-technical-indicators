...
def calcAroon(high, low, length):
    ...
    upper = data.rolling(length+1).apply(lambda x: x.argmax()) / length * 100
    downer = data_l.rolling(length+1).apply(lambda x: x.argmin()) / length * 100
...
def GetCustomSignal():
        ...
        ##
        df1['aUP'],df1['aDN'] = calcAroon(df1['High'],df1['Low'], 14)
